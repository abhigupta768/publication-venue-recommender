...
class hanLSTM(nn.Module):
    def __init__(self, doc_len, text_len, vocab_size, embed_dim, word_hidden_size,
        ...
        for i in range(self.doc_len):
            ...
            self.sent_wise_attlstms.append(Attention(2*self.word_hidden_size))
        self.doc_lstm = nn.Sequential(lstm_cell(2*self.word_hidden_size, self.sent_hidden_size), nn.Dropout(p=self.dropout))
        self.doc_attention = Attention(2*self.sent_hidden_size)
        self.title_lstm = nn.Sequential(lstm_cell(self.embed_size, self.title_hidden_size), nn.Dropout(p=self.dropout))
        self.title_attention = Attention(2*self.title_hidden_size)
        self.final_att = Attention(2*self.sent_hidden_size)
        self.linear_stack = nn.Sequential(nn.Linear(2*self.sent_hidden_size, self.linear_out_size_2), nn.ReLU(), 
    ...
    def forward(self, x, xt):
        ...
        att_doc_lstm = self.doc_attention(doc_lstm).unsqueeze(1)
        title_lstm = self.title_lstm(xt_embed)
        att_title_lstm = self.title_attention(title_lstm).unsqueeze(1)
        final = torch.cat((att_doc_lstm, att_title_lstm), dim=1)
        final_att = self.final_att(final)
        y_ = self.linear_stack(final_att)
        # y=F.softmax(y_)
        return y_
