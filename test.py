...
Original file is located at
    https://colab.research.google.com/drive/1lfBV7WYSwGPf5IqGaWONGbvCxctXJUDk
"""
...
model.cuda()
model.load_state_dict(torch.load('./trained_models/0.39606141672099454_checkpoint.pt')['model'])
...
def calc_f1(y_pred_top, y_true, top):
  precisions = []
  recalls = []
  for i in range(300):
    count_r = 0
    count_rt = 0
    for j, label in enumerate(y_true):
      if label==i:
        count_rt+=1
        if(y_pred_top[j][:top].__contains__(label)):
          count_r+=1
    recalls.append(count_r/count_rt)
    count_p = 0
    count_pt = 1
    for j, pvs in enumerate(y_pred_top):
      if(y_pred_top[j][:top].__contains__(i)):
        count_pt+=1
        if(y_true[j]==i):
          count_p+=1
    precisions.append(count_p/count_pt)
  mean_p = sum(precisions)/300
  mean_r = sum(recalls)/300
  print(mean_p, mean_r)
  f1 = (2*mean_p*mean_r)/(mean_p+mean_r)
  return f1

f1_9 = calc_f1(y_pred_top, y_true, 15)
